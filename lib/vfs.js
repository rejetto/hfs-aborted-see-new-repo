...
Vfs.prototype.readFrom = function(stream, cb) {
    var v = yaml.load(fs.readFileSync(stream));
...
FileNode.prototype.__defineSetter__('parent', function(newP){
    ...
    try { this._parent.children.removeItems(this) }
    catch(e) {}
    // bind this way
...
FileNode.prototype.dir = function(cb/*filenodes,bads*/) {
    ...
    function addVirtuals(){
        ...
        This.children.forEach(function(node){
            ...
            if (node.nodeKind === NK.FIXED) {
                var k = normalizeFileName(node.name);
                if (k in fnodes)
                    node.overlapping = true; // overlapping, note it.
                fnodes[k] = node; // put
                return;
            }
            ...
            // keep note of overlapping
            var now = normalizeFileName(node.name);
            if (now in fnodes)
    ...
    function Return() { cb(fnodes._values(), bads) }
}; // dir

FileNode.prototype.reconsiderNodeKind = function() {
    // is this node storing any modification
    var anyMod = this.resource && (
        this.customName || !isEmpty(this.permissions) || !isEmpty(this.deletedItems)
    );
