...
Vfs.prototype.fromString = function(y, cb) {
    var v = yaml.load(y);
    // from simple object to FileNodes
    this.root = (function recur(n, cb){
        return new FileNode(n.nodeKind, n.resource||n.customName, n.customName ? { name: n.customName } : {}, function(err,res){
            if (err) return cb(n);
            if (!n.children) return;
            n.children.forEach(function(c){
                recur(c, cb).parent = res;
            });
            cb(null, this.root);
        });
    })(v, cb||idFun);
    ...
    function addVirtuals(){
        ...
        This.children.forEach(function(node){
            ...
            if (node.nodeKind === NK.FIXED)
                return node.refreshOverlapping(fnodes);
            ...
            node.refreshOverlapping(fnodes);
            delete fnodes[was];
        });
        Return();
    }
...
FileNode.prototype.refreshOverlapping = function(siblings/*optional*/, cb) {
    assert(this.nodeKind._among(NK.MOD,NK.FIXED)); // this makes no sense otherwise
    var s = siblings; // shorcut
    if (isFunction(s))
        cb=s, s=0;

    var self = this; // going async, preserve scope
    if (s) // dirty job ready made, this will be synchronous
        return doIt(s);
    if (caseSensitiveFileNames) // optimization
        return fs.exists(this.parent.resource.includeTrailing('/')+this.name, function(exists){
            this.overlapping = exists;
            cb && cb();
        });
    this.parent.dir(doIt);

    function doIt(items) {
        var k = normalizeFileName(self.name);
        self.overlapping = k in items;
        items[k] = self;
        cb && cb.call(self);
    }
};//refreshOverlapping
