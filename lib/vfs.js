...
function isPathAndSubpath(path_, sub) {
    ...
    return path_ && sub && misc.sameFileName(path_, sub.substr(0, path_.length));
} // isPathAndSubpath
...
FileNode.prototype.dir = function(cb/*filenodes,bads*/) {
    ...
    async.map(directoryLoaded, toNode, function(err, fnodes){
        ...
        var bads = fnodes.removeItems(L('typeof A==="string"'), true);
        // to avoid other entries with same name, build an object to find them quickly
        var fname2idx = fnodes.toObject('[A.name, B]');
        ...
        (This.children||[]).forEach(function(e){
            ...
            var isMod = e.nodeKind===NK.MOD;
            var idx = fname2idx[isMod ? e.getExpectedName() : e.name]; // search an element with same name..
            fnodes[idx===undefined ? fnodes.length : idx] = e; // and overwrite, or add if not found
            if (isMod && e.customName) { // it may eventually overlap another node, if customName is already present in the folder
                e.overlapping = true; // communicate the fact
                fnodes.splice(fname2idx[e.customName], 1); // remove the overlapped node
            }
        });

        cb(fnodes, bads); // finally returns (asynchronously)
    });
}; // dir
