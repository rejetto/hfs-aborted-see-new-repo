...
FileNode.prototype.dir = function(cb/*filenodes,bads*/) {
    ...
    if (!this.isOnDisk())
        return addVirtuals(); // virtual folder, nothing to load, skip to next phase
    ...
    function addVirtuals(){
        ...
        This.children.forEach(function(vnode){ // virtual node
            ...
            if (vnode.nodeKind === NK.FIXED) {
                var k = normalizeFileName(vnode.name);
                if (k in fnodes)
                    vnode.overlapping = true; // overlapping, note it.
                fnodes[k] = vnode; // put
                return;
            }
            ...
            vnode.stats = f.stats; // keep the fresher one
            // keep note of overlapping
            var now = normalizeFileName(vnode.name);
            if (now in fnodes)
                vnode.overlapping = true;
            // replace
            fnodes[now] = vnode;
            delete fnodes[was];
        });
