...
FileNode.prototype.containingFolderOf = function(child){
    return pathAndSubpath(this.resource, child.resource);
};

function pathAndSubpath(path_, sub) {
...
FileNode.prototype.getFolder = function(cb){
    var p = this.parent;
    return this.isTemp() && !p.containingFolderOf(this)
        ? new FileNode(NK.TEMP, dirname(this.resource), {parent:p}, cb) // we must create a temp node
        : cb(p);
};
...
FileNode.prototype.resourceOnlyRelativeTo = function(other){
    return other.containingFolderOf(this)
...
FileNode.prototype.add = function(what, cb/*optional*/) {
    var restored = isPath(what)
        && this.deleted
        && pathAndSubpath(this.resource, what)
        && this.restoreDeleted(path.basename(what));
    new FileNode(dbg('restored', restored) ? NK.TEMP : NK.FIXED, what, {parent:this}, cb); // create child
    return this;    
}; // add
...
FileNode.prototype.restoreDeleted = function(name) {
    ...
    this.deleted.splice(i,1); // remove from the array
    return true;
}; // hasDeleted
...
FileNode.prototype.dir = function(cb) {
    ...
    var toNode = function(filename,cb){ // our createFileNodeFromRelativeUri doesn't comply to the standard "cb(err,result)" form, so we need to encapsulate it
        var sameAsFilename = misc.sameFileName.bind(null,filename);
        _this.deleted.any(sameAsFilename)
    ...
    switch (this.nodeKind) {
        case NK.TEMP:
            ...
            this.getFolder(function(fldr){
                assert(fldr, 'root?');                
                fldr.deleted.push(path.basename(_this.resource));
