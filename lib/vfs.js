    ...
    function addVirtuals(){
        ...
        This.children.forEach(function(vnode){ // virtual node
            ...
            if (vnode.nodeKind === NK.FIXED) {
                ...
                if (k in fnodes) vnode.overlapping = true; // overlapping, note it.
                fnodes[k] = vnode; // put
                return;
            }
            ...
            // we don't add the vnode, just apply the mod
            var f = fnodes[was];
            f.name = vnode.name;
            // keep note of overlapping
            if (now in fnodes) f.overlapping = true;
            fnodes._remapKeys(was, now); // update index
        });
        Return();
    }
