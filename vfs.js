...
FileNode.prototype.setPath = function(what, cb/*optional*/) {
    ...
    if (this.name != '/') { // this is the root, you cannot change its name by binding it
        this.name = null;
    }
...
FileNode.prototype.dir = function(cb) {
    ...
    var items = {};
    var asyncLeft = 0;    
    var isItOver; // this is what has to be done when all async calls are done
    directoryLoaded.forEach(function(e){
        ++asyncLeft;
        _this.createFileNodeFromRelativeUri(e, function(fn){
            --asyncLeft;
            items[e] = fn;
            isItOver();
        });
    ...
    isItOver = function(){
        if (asyncLeft) return; // the time has not come yet
        
        this.getChildren().forEach(function(e){
            items[e.name] = e;
        });

        // now we have another turn of async calls, so let's redefine what must be done when they are done        
        isItOver = function(){
            if (asyncLeft) return; // the time has not come yet 
            cb(items);
        }; // isItOver
        ...
        // collect more data about items, and pass them into reply.items as an array
        items.forEach(function(e, name) {
            if (!e.isOnDisk()) return;
            ++asyncLeft; // keep track of async calls we are waiting to return 
            fs.stat(e.resource, function onStat(err,stats){
                --asyncLeft;
                if (err) {
                    delete items[name];
                }
                    ...
                    e.stats = stats;
                    e.itemKind = stats.isDirectory() ? FK.FOLDER : FK.FILE;
                }    
                isItOver();
            });//stat
        });//forEach
